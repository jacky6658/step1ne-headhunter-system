...
## 🚫 外籍名字雙重篩選（第二層：AI Bot 驗證）

> **重要**：系統在匯入時已自動進行第一層外籍名字篩選（基於華人姓氏白名單）。
> AI Bot 負責**第二層驗證**，處理系統可能漏判的情況。

### AI Bot 評分前必做檢查

在對候選人進行 AI 評分之前，先檢查名字：

1. **名字明顯是外國人**（如 John Smith、Michael Johnson、Sarah Williams）→ PATCH 狀態為 `外籍已過濾`
2. **名字是華人但系統誤判為外國人**（status 已是 `外籍已過濾`，但名字其實是華人拼音）→ PATCH 改回 `爬蟲初篩`
3. **名字不確定**（如混血名字、少見姓氏）→ 保持原狀態，繼續評分流程
...
> ⚠️ 這是最常混淆的兩個欄位，請務必按照以下規則操作。

### `notes`（備註紀錄）— 自由文字，AI 分析報告用

**用途：** 非結構化的文字備註、AI 分析摘要、背景資訊

| 應寫入 notes 的內容 | 範例 |
|-------------------|------|
| 候選人特質摘要 | `"7年資深工程師，台大碩士，穩定性高，主動求職中"` |
| 目標職缺標記 | `"目標職缺：Java Developer | Bot 履歷匯入 | 2026-03-03"` |
| 特殊背景說明 | `"曾任 CTO，現轉型 IC，薪資期望 180-220K"` |
| 顧問評語 | `"[2026-03-03] Jacky-aibot：候選人技術深度佳，建議優先聯繫"` |
| AI 履歷分析報告 | 穩定性評分依據、優勢缺口分析等 |

**API 操作：**
```json
PATCH /api/candidates/:id
{ "notes": "你的備註文字", "actor": "Jacky-aibot" }
```

> ⚠️ `notes` 是**整個覆蓋**。若要追加而非覆蓋，先 GET 取得現有內容，append 後再 PATCH。
> 推薦格式：`[現有內容]\n[今日日期] [你的身份]：[新增內容]`

---

### `progressTracking`（進度追蹤）— 結構化，招募里程碑用

**用途：** Pipeline 階段的里程碑事件，供顧問追蹤招募流程
...
**絕對不要把以下內容寫進 progressTracking：**
- ❌ 自由文字備註（請用 `notes`）
- ❌ 候選人背景資訊（請用 `notes`）
- ❌ AI 評分報告（請用 `ai_match_result` + `notes`）
