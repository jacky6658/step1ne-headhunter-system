...
```
POST https://api-hr.step1ne.com/api/jobs
...
**Q: 職缺已存在（同公司同職位）要怎麼辦？**
A：先呼叫 `GET https://api-hr.step1ne.com/api/jobs`，搜尋 `position_name` 是否已存在。若已存在，改用 `PUT https://api-hr.step1ne.com/api/jobs/{id}` 更新欄位，不要重複新增。
...
```
請讀取以下文件後立即執行職缺匯入任務：
https://api-hr.step1ne.com/api/jobs-import-guide
