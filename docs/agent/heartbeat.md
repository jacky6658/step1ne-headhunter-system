...
### 🤖 AI 顧問閉環分析（每 4-6 小時輪流）

**這是核心閉環任務 — 自動對有履歷的人選執行 AI 深度分析**

#### 前置條件
- 獵頭系統 API 健康（`GET http://localhost:3003/api/candidates?limit=1` 回 200）
- 人選必須有履歷 PDF 附件（`resumeFiles` 不為空）— **沒有履歷的一律跳過**
...
1. **掃描待分析人選**：
   ...
   ```
   篩選條件：有 `targetJobId` + 有 `resumeFiles` + 沒有 `aiAnalysis`（尚未分析過的）
...
2. **對每個待分析人選，依序執行**：
   ...
   e. **使用提示詞 + 人選資料 + 履歷 + 職缺，產出完整 AI 顧問分析**：
...
- 記錄 `lastAlert.ai_analysis_results`：最近一次分析的成功/跳過/失敗數量
- 如果連續 3 次全部跳過（無新的待分析人選）→ 降低檢查頻率到每日 2 次
